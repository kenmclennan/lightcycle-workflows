...
- The engine defines the grammar and the hook catalog; the built-in bundles (`workflows/spec-driven.md` + its `steps/*.md`) are the working reference to model from. `lc workflow add` and `lc workflow check` validate a bundle - fix what they name, do not work around them. A worker's `lc` cannot run these against its own live home (the `workflow` verb is refused there); the `simulate` CI job (`.github/workflows/simulate.yml`) runs them in a throwaway CI home on every push and is the validation a worker relies on - see `build-workflow.md` / `review-workflow.md`.
