...
- **Never read the lightcycle engine source to understand the grammar.** The grammar, hook catalog, graph/contract model, and CLI behaviour come ONLY from the `lightcycle:author-workflow` skill and this origin's existing bundles (read `workflows/spec-driven.md` and the `steps/*.md` as working models). A distributed setup is the pipx engine **binary** + the plugin + this origin - NOT the engine's source code - so a workflow designed by reading engine internals (`graph.py`, `cli.py`, `contracts.py`, `domain/`) cannot be reproduced by anyone else, and silently makes the whole authoring path depend on source access. If the skill or the existing bundles lack something you need, that is a gap to **report**, not a reason to open the engine source.
