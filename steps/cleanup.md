...
1. CLAIM: `lc claim agent`. If nothing, say "no work" and EXIT. The printed JSON is your step; take `.id` as STEP, `.item` as ITEM, and `.repo_path` as CODE_PATH.
...
4. If anything refuses - a worktree that will not remove, a branch that is not merged - do NOT force it: `lc set STEP --state blocked --needs "<what refused and what you tried>" --reason "<what happened that led here>"`, then EXIT. A half-torn-down item is worse than one left intact for a human to look at.
