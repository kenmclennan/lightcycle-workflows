...
1. CLAIM: `lc claim agent`. If nothing, say "no work" and EXIT. The printed JSON is your step; take `.id` as STEP, `.parent` as ITEM, `.workspace` as WORKSPACE, `.branch` as BRANCH, and `.spec_path` as SPEC (an absolute path to the design spec, which lives in the engine - NOT inside the worktree).
...
9. **Recording a design-changing rework as a spec amendment.** This only applies when either (a) `lc show STEP`'s note contains a `SPEC CHANGE:` line, or (b) mid-implementation you find the change you are building contradicts, rather than extends, a stated spec Design/Acceptance decision - narrower than the ordinary "spec can be stale" posture (step 3): this applies only when a human's own comment or review thread on the code PR (not your own inference) already asked for the contradicting change. Neither path is your own judgement that the spec is wrong - both terminate in a real, findable PR comment or review thread that asked for the change:
   ...
   **If neither path turns up a real, resolvable authorizing url, do not amend.** Block instead: `lc set STEP --state blocked --needs "<what human decision this needs>"`, then EXIT. Only once a genuine authorizing comment/thread is in hand does this pass amend the spec, before finishing:
   ...
   If the push in step 3 above is rejected (a concurrent amendment landed), fetch, re-apply the appended section against the new tip, and retry once; if it still conflicts, `lc set STEP --state blocked --needs "<...>"`.
...
12. Reflect: `lc attach STEP feedback "<text>"`. Freeform - a grammar gap this file didn't cover, a tooling/environment friction (a blocked command, a wrong assumption), anything that would make the next build-workflow pass smoother. Skip only if truly nothing.
