...
4. Write PURE gherkin `.feature` files - Feature / Scenario / Given-When-Then, `Scenario Outline`
   ...
   * **Tag every scenario `@wip`** (or the repo's documented skip tag) so CI skips them until the code exists - a red suite must never land on the feature PR. If the repo has no skip convention, note it in your feedback and use `@wip`.
