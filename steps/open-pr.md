...
3. IDEMPOTENCY CHECK: `lc show ITEM` - if the item already has a `pr` artifact, the PR exists. Run `git push --force-with-lease` to ensure the branch is current, then skip to step 7.
