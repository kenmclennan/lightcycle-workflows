...
1. CLAIM: `lc claim agent`. If nothing, say "no work" and EXIT. The printed JSON is your step; take `.id` as STEP, `.item` as ITEM, `.workspace` as WORKSPACE, `.branch` as BRANCH, and `.phase` as PHASE.
...
3. BRANCH-AUTHORITATIVE IDEMPOTENCY CHECK: the branch, not the `.pr` this phase run already carries, decides whether a PR for this phase already exists. `gh pr list --head BRANCH` (the same query step 6 uses):
   ...
   - **No PR is returned, and the claim JSON's `.pr` is empty:** genuinely no PR for this phase yet - continue to step 4 and open one there, same as before.
   - **No PR is returned, but the claim JSON carries a `.pr`:** this is neither "exists" nor "absent" - it is a divergence, not a case to resolve either way. A rework pass can re-mint this worktree's branch while the run's recorded PR still points at the earlier branch name. Do not silently reuse it (it is not this branch's PR) and do not silently open a new one (this phase run already has a PR somewhere, and opening another duplicates it). Instead: `lc set STEP --state blocked --needs "this phase run's recorded PR <the .pr url> does not match its worktree's branch BRANCH (gh pr list --head BRANCH found nothing) - resolve the divergence before opening or reusing a PR for this phase." --reason "<what happened that led here>"`, then EXIT.
