...
5. If ANY item needs rework, route WATCHED once (not once per item): `lc done WATCHED changes --note "<summary of what needs to change, with file:line where relevant>"` - the write-code agent picks this up on WATCHED's next push.
   - **`feedback-spawned-through` is the engine's, not yours - but it sits beside yours on the same node.** `monitor_prs` writes it on WATCHED, recording the newest comment it has already spawned a handle-feedback for so the pool does not spawn twice for one comment. You write `feedback-watermark` on WATCHED too. Two writers, one node, and in practice the same epoch. **Read and advance only your own**; do not copy one into the other and do not report finding the engine's where you looked for yours - it is not a missing value. That the two carry one value under two names is a real duplication and is filed; until it is reconciled, treat the engine's as none of your business.
