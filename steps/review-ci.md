...
2. Decide: fix it by hand and push to the branch yourself; leave it blocked; abandon the item; or re-arm the coder - `lc new step "write-code: <title>" --parent ITEM` (or `lc set STEP --state ready` style unblock) once you know what should change, with a note on what to try differently so the next write-code pass does not repeat the same failure.
