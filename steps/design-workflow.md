...
1. CLAIM: `lc claim design-workflow`. If nothing, say "no work" and EXIT. The printed JSON is your step; take `.id` as STEP, `.parent` as ITEM, `.workspace` as WORKSPACE, `.branch` as BRANCH, `.repo_path` as CODE_PATH, and `.brief` as BRIEF (the literal text, not a path).
