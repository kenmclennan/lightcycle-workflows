...
5. Outcome: pass or fail, first resolve the PR - the claim JSON's `.pr` (this pass's phase run holds it); if absent, `gh pr list --head BRANCH --json url -q '.[0].url'`. **Re-pull head first**: `git fetch origin` again immediately before posting, and if `origin/BRANCH` advanced, re-check the changed files - a push may have landed mid-review. `feature-watch-ci` has already confirmed CI concluded clean for this head SHA before this step was reached - do not re-derive that state. Then post a `gh pr comment <pr> --body "<!-- lc --> ..."` before (or as part of) the `lc done`/`lc set` call:
