...
3. Review the `.feature` files this branch added against the spec:
   ...
   - **No unreconciled scenario removal.** For every `.feature` file this branch's three-dot diff shows as modified (not one it only added), compare per-title `Scenario:`/`Scenario Outline:` occurrence counts between `origin/main`'s copy of the file and this branch's tip - `grep -E '^\s*(Scenario|Scenario Outline):' <file>` on each side, title text trimmed, counting duplicates rather than deduping to a set (a same-titled duplicate surviving would otherwise mask a real removal). A title whose count dropped is a scenario defect unless the feedback this pass is answering (the PR's comment/review thread) names it: directly, as something to change or remove, or paired with a specific added title as an explicit rename (both titles present in the text) - no fuzzy matching beyond that, never inferred from plausibility or diff size. This check is inert - nothing dropped - on a first-pass PR that only adds scenarios, by construction; it only bites a rework round.
4. Reflect: `lc attach STEP reflection "<text>"`. Freeform - a thin or ambiguous spec that made coverage hard to judge, a recurring gap in the scenarios, or "clean". Honest sentences, not a checklist; skip only if truly nothing.
5. Outcome: pass or fail, first resolve the PR - the claim JSON's `.pr` (this pass's phase run holds it); if absent, `gh api "repos/{OWNER}/{REPO}/pulls?head={OWNER}:{BRANCH}&state=open" --jq '.[0].html_url // empty'` (`OWNER`/`REPO` from `git remote get-url origin` in WORKSPACE). **Re-pull head first**: `git fetch origin` again immediately before posting, and if `origin/BRANCH` advanced, re-check the changed files - a push may have landed mid-review. `feature-watch-ci` has already confirmed CI concluded clean for this head SHA before this step was reached - do not re-derive that state. Then post a `gh api repos/{OWNER}/{REPO}/issues/{NUM}/comments -f body="<!-- lc --> ..."` (`-f`, inline string; `NUM` parsed from the resolved PR url) before (or as part of) the `lc done`/`lc set` call:
   - Pass: step 3 found no scenario defects. Comment names what was checked (coverage of the spec, depth, `@wip` tags) and the clean verdict, THEN `lc done STEP done` (-> feature-await-merge).
   - Fail: a scenario defect from step 3 (missing/thin/malformed scenarios, or an unreconciled removal). Comment states exactly which scenarios are missing, thin, malformed, or dropped without justification (naming the dropped title(s)), THEN `lc done STEP rejected --note "<what to change>"` (-> feature-writer; the note forwards onto the next feature-writer step).
   - Cannot review the scenarios: `lc set STEP --state blocked --needs "<...>" --reason "<what happened that led here>"`, no PR comment.
