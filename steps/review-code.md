...
3. Review against the spec's acceptance criteria - each check it lists, or its stated intent if it has no checklist; verify by running tests/build, not by reading alone.
   ...
   - Fast path: if the diff touches only `steps/*.md`, other `*.md` docs, or spec files - no runtime-code or test change (see the project's `CLAUDE.md`, read per step 2, for its layout) - verify lint plus a quick sanity (e.g. `lc workflow check <origin>/<name>` still composes if steps changed) instead of the full suite.
